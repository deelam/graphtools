...
public class ClassDepend {
  ...
  public static ClassDepend newInstance(String classpath, String platform, boolean warn)
    ...
    ClassDepend classDepend =
        !warn ? new ClassDepend(classpath, platform) : new ClassDepend(classpath, platform) {
  ...
  public static void main(String[] args) {
    try {
      ...
      Set result =
          classDepend.filterClassDependencyRelationShipMap(
              classDepend.getDependencyRelationshipMap(rootClasses, recurse), cdp);
      Iterator results = result.iterator();
      while (results.hasNext()) {
        Object rezult = results.next();
        if (!(rezult instanceof ClassDependencyRelationship))
          continue;
        ...
        if (graph) {
          ...
          Iterator<ClassDependencyRelationship> itr = deps.iterator();
          while (itr.hasNext()) {
            ClassDependencyRelationship dep = itr.next();
            if (result.contains(dep)) {
              System.out.println( 
                  new File(cl.getJarfile()).getName()
                  + ", " + ((dep.getJarfile()!=null)?new File(dep.getJarfile()).getName():dep.getJarfile())
                  + ", " + cl.getFullyQualifiedClassName() 
                  + ", " + dep.getFullyQualifiedClassName() 
                  );
              //              System.out.println("\"" + cl + "\" (" + cl.getJarfile() + ") -> " + "\"" + dep
              //                  + "\" (" + dep.getJarfile() + ")" + ";");
            }
          }
        }
  ...
  ClassDepend(String classpath, String platform) throws MalformedURLException, IOException {
    ...
    loader =
        (systemClasspath.equals(classpath)) ? system : new URLClassLoader(
            getClasspathURLs(classpath), parent);
    packageClasses = new PackageClasses(classpath);
    platformLoader =
        (platform == null) ? parent : new URLClassLoader(getClasspathURLs(platform), parent);
  ...
  public Map getDependencyRelationshipMap(Collection rootClasses, boolean recurse)
    ...
    Map<String, ClassDependencyRelationship> result = new HashMap<>(); // May be changed to ConcurrentHashMap for Java 5
    ...
    while (!compute.isEmpty()) {
      ...
      while (computeIterator.hasNext()) {
        ...
        if (!seen.contains(name)) {
          ...
          if (recurse) {
            // System.out.println("1: "+loader.getResource(resource));
            result.get(name).setUrl(loader.getResource(resource));
            InputStream in = loader.getResourceAsStream(resource);
            if (in == null) {
              //noteClassNotFound(name);
              System.out.println("2: not found: " + resource);
            } else {
              try {
  ...
  public Pattern createPattern(Collection names) {
    ...
    while (namesItr.hasNext()) {
      ...
      if (name.endsWith(".*")) {
        sb.append(quote(name.substring(0, name.length() - 1)) + "[^.]+");
      } else if (name.endsWith(".**")) {
        sb.append(quote(name.substring(0, name.length() - 2)) + ".+");
      } else {
