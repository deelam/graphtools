...
public class DefaultPopulator implements Populator{
	...
	public static Edge importEdge(Graph graph, Edge e, Direction direction, Vertex v1inGraph, Vertex v2inGraph){
		...
		if(newEdge == null){
			...
			checkArgument(newEdge.getVertex(direction).equals(v1inGraph), "Expecting " + v1inGraph + " but got "
	...
	public static void copyProperties(Element fromE, Element toE){
		for(String key:fromE.getPropertyKeys()){
			...
			if(toValue == null){
				...
				log.warn("Appending property values for '{}' origValue={} newValue={}", key, toValue, fromValue);
